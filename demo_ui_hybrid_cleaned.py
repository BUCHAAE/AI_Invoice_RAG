...
if not attendance_df.empty:
    try:
        ...
        print(f"✅ First attendance date parsed as: {first_date.strftime('%d %B %Y')} ({first_day})")
        print(f"✅ First attendance date parsed as: {first_date.strftime('%d %B %Y')} ({first_day})")
print(f"✅ First attendance date parsed as: {first_date.strftime('%d %B %Y')} ({first_day})")
...
documents.append(f"The total cost for all invoices is ${total_cost:.2f}.")
attendance_df['ParsedDate'] = pd.to_datetime(attendance_df['Date'], format='%d/%m/%Y', errors='coerce')
attendance_years = sorted(attendance_df['ParsedDate'].dt.year.dropna().unique())
documents.append("Snoopy attended doggy daycare in the following years: " + ", ".join(str(y) for y in attendance_years))
...
print("🔢 Creating vectorstore with embeddings...")
